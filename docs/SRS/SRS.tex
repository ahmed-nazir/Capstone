...
\begin{itemize}
  ...
  \item The device is turned on and test functioning can begin
  
  \item The device would be modular so depending on what test is required the device would be able to change from testing shock, vibrations, temperature, and humidity by just switching the module into the device.
  
  \item The device will continually send receive information through an Arduino board, the values from the Arduino board will then get converted into a desktop application
  
  \item The converted values from the desktop application will be sent to a remote database where the user will be able to access the data and use the information for what it is needed for.
  
  \item The device will be able to tell the user if the equipment being tested is going above or is below the equipment threshold. This will allow the user to stop the testing before any important components get compromised on the unit.

  \item In event of the the devicee turning of prematurely the data will be automatically saved on the database and stored for later use.
  
  \item The user can then access the data on a website where all the info will be stored through our database.
  
\end{itemize}

\subsection{User Consideration}
The design of the device is made with the user in mind, our users will be mainly engineering students trying to collect data on equipment they have designed. Since this is the case they will be able to customize the output parameters for what they need such as the gain and or the initial value of the test. This will allow for a more fluid way to setup the test for them. Since it is student run club we would like to make cost effective as well, other data collecting devices are expensive and are
\subsection{Impact}
...
\begin{enumerate}
  ...
  \item \textbf{Presentations:} In order to master presentating and demonstrating the device you can take advice from difference on audiences what they like when someone is presenting something to them. The main focus is to keep the audience engaged and makesure they understand the presentation, feedback from different audiences will always help since you will see what audiences like to when watching a presentation and what keeps them engaged. Another way to master the skill of presentation is to watch notable presenters and speakers and see how they keep their audiences engage and make it easy for them to understand. For our method we will look at other presentations and critique what they did well and what they could improve and see how we could improve our presentation and what to avoid when presenting.
